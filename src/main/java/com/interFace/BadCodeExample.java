...
// BadCodeExample: 인터페이스를 사용하지 않은 예시
public class BadCodeExample {
	private final UserServiceImpl userService;

	public BadCodeExample(UserServiceImpl userService) {
		this.userService = userService;
	}
	/* 만약 다른 구현체를 사용하려면
	 * 	public BadCodeExample(OtherServiceImpl userService) {
			this.userService = userService;
		}
	 * 이렇게 될텐데 이렇게되면 주입받는 OtherServiceImpl 부분을 변경해야되기 때문에 해당클래스를 변경하는것이 됨!
	 * */

	public void addUser(User user) {
		userService.addUser(user); // UserServiceImpl 클래스에 직접 의존
	}
}
/*
